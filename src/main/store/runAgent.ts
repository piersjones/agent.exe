...
const promptForAction = async (
  ...
  const message = await anthropic.beta.messages.create({
    ...
    system: `The user will ask you to perform a task and you should use their computer to do so. After each step, take a screenshot and carefully evaluate if you have achieved the right outcome. Explicitly show your thinking: "I have evaluated step X..." If not correct, try again. Only when you confirm a step was executed correctly should you move on to the next one. Be mindful that you may make mistakes when opening apps, clicking into text areas or interacting with windows. After each step, carefully evaluate whether the action has led to the desired outcome. Note that you have to click into the browser address bar before typing a URL. You should always call a tool! Always return a tool call. Remember call the finish_run tool when you have achieved the goal of the task. Do not explain you have finished the task, just call the tool. Use keyboard shortcuts to navigate whenever possible.`,
